...
#define YY_LOG_LEVEL(logger, level)  \
    if(logger->getLevel() <= level) \
        yy::LogEventWrap(yy::LogEvent::ptr(   \
                        new yy::LogEvent(logger, __FILE__, __LINE__, 0,  \
                        yy::GetThreadId(), yy::GetFiberId(), time(0), level)  \
                    )   \
                ).getSS()
...
//使用单例模板声明一个单例的管理器
typedef yy::Singleton<LoggerManger> loggerMgr;	//懒汉模式
```
...
```c++
#ifndef __YY_SINGLETON_H__
#define __YY_SINGLETON_H__
#include<memory>
namespace yy{
...
int main(){
    ...
    YY_LOG_INFO(l) << "xxxxx";
	//结果：
    //2022-10-27 10:08:26     21020   0       [INFO]  [root]  /home/ubuntu/StudyCpp/yy/tests/test.cpp:25   xxxxx
    return 0;
}
YY_LOG_INFO(l) << "xxxxx";    --->   #define YY_LOG_INFO(logger) YY_LOG_LEVEL(logger, yy::LogLevel::INFO)
#define YY_LOG_LEVEL(logger, level)  \
    if(logger->getLevel() <= level) \
        yy::LogEventWrap(yy::LogEvent::ptr(   \
                        new yy::LogEvent(logger, __FILE__, __LINE__, 0,  \

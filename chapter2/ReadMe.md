...
YAML::Node root = YAML::LoadFile("/home/yyshinidaye/yy/workspace/bin/conf/log.yaml");
...
> 读入yaml，调用yy::Config::LoadFromYaml(root);将yaml结点展平为map格式，键为字符串，值为yaml结点。找到在s_datas中存储过的进行更新，需要进行反序列化fromString，从string中恢复原有类型存入ConfigVar的m_val中。
...
});	//old_val=[Person name =  age = 0 sex = 0 ]new_val=[Person name = yy age = 35 sex = 1 ]

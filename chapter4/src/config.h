...
public:
    ...
    std::string toString() override {
        try{
            ...
            YY_LOG_ERROR(YY_GET_ROOT()) <<"ConfigVar::toString exception" 
    ...
    bool fromString(const std::string& val) override {
        try{
            ...
            YY_LOG_ERROR(YY_GET_ROOT()) <<"ConfigVar::fromString exception"
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp){
                YY_LOG_ERROR(YY_GET_ROOT()) << "Lookup name=" << name << "exists";
                return tmp;
            }
            else{
                YY_LOG_ERROR(YY_GET_ROOT()) << "Lookup name =" << name << " exists but type: not " 
                    << typeid(T).name() << " real_type = " << it->second->getTypeName()              //getTypeName():基类指针不能访问派生类成员函数，必须声明虚函数
                    << " " << it->second->toString();     
                return nullptr;
            }
        }
        if(name.find_first_not_of("abcdefghijklmnoporstuvwxyz._012345678")
                    != std::string::npos){
                YY_LOG_ERROR(YY_GET_ROOT()) << "Lookup name invaild " << name;

...
#define YY_LOG_LEVEL(logger, level)  \
    if(logger->getLevel() <= level) \
        yy::LogEventWrap(yy::LogEvent::ptr(   \
                        ...
                        yy::GetThreadId(), yy::GetFiberId(), time(0),  \
                        yy::Thread::GetName(), level)  \
                    )   \
                ).getSS()
...
//在其他文件获取logger的智能指针
#define YY_GET_ROOT() yy::loggerMgr::GetInstance()->getRoot()
...
typedef yy::Singleton<LoggerManger> loggerMgr;

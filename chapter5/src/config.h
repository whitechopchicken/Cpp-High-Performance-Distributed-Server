...
public:
    ...
    std::string toString() override {
        try{
            ...
            YY_LOG_ERROR(YY_LOG_ROOT()) <<"ConfigVar::toString exception" 
    ...
    bool fromString(const std::string& val) override {
        try{
            ...
            YY_LOG_ERROR(YY_LOG_ROOT()) <<"ConfigVar::fromString exception"
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp){
                YY_LOG_ERROR(YY_LOG_ROOT()) << "Lookup name=" << name << "exists";
                return tmp;
            }
            else{
                YY_LOG_ERROR(YY_LOG_ROOT()) << "Lookup name =" << name << " exists but type: not " 
        ...
        if(name.find_first_not_of("abcdefghijklmnoporstuvwxyz._012345678")
                    != std::string::npos){
                YY_LOG_ERROR(YY_LOG_ROOT()) << "Lookup name invaild " << name;

...
void Logger::setFormatter(const std::string& val){
    yy::LogFormatter::ptr new_val(new yy::LogFormatter(val));
    if(new_val->isError()){
        return ;
    }
...
yy::ConfigVar<std::set<LogDefine> >::ptr g_log_define = 
    yy::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_define->addListener([](const std::set<LogDefine>& old_val,
                    const std::set<LogDefine>& new_val){
            YY_LOG_INFO(YY_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_val){
                auto it = old_val.find(i);
                yy::Logger::ptr logger;
                if(it == old_val.end()){
                    //新增logger
                    logger = YY_LOG_NAME(i.name);
                }
                else{
                    if(!(i == *it)){
                        //修改logger
            ...
            for(auto& i : old_val){
                ...
                if(it == new_val.end()){
                    //删除logger
                    auto logger = YY_LOG_NAME(i.name);

互斥量的实现用pthread库， 其它的用c++11的thread
# 网络框架
...
## 线程同步的一些机制简单总结一下：  
```
1. 互斥锁 mutex：   只有持有锁的线程可以访问被保护的资源，其他线程必须等待锁的释放。
2. 读写锁 rwlock：  允许多个线程同时读取共享资源，但是只允许一个线程进行写的操作==>提高read操作的并发性；
3. 条件变量 condition variable：    用于在线程间进行通信，允许一个线程等待特定条件的发生，而其他线程可以通知条件的发生，与mutex一起使用。
==>用于线程等待和唤醒机制。
4. 信号量 Semaphore：   一个计数器，用于控制对共享资源的访问。可以用于限制同时访问资源的线程数量，或者用于线程间的通信
5. 原子操作 atomic operation：  让某一种操作不会被其他线程中断。相比于mutex，atomic更适用于简单的变量操作
6. 自旋锁 spin lock：   在获取失败时，会一直自旋等待的锁。 
==>适用于短时间内持有锁的情况，可以避免线程《进入睡眠状态》和《唤醒状态》的开销
7. 屏障 Barrier：   用于《在多个线程中同步》，要求所有线程都达到某个点才能继续执行。
==>适用于分阶段的并发任务
```
...
> c++11 可以用std::lock_guard来代替手动实现 ScopedLock 作用域锁
